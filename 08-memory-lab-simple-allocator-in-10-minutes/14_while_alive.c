/* Build a Simple Memory Allocator: Understanding malloc and new - slide 14: thank you (C version of 14_while_alive.cpp) */
...
/* C has no std::vector: a growable array, grown with realloc and freed by hand */
...
int main(void) {
    ...
    int ok = 1;
    while (alive()) {
        ok = ok && push_back(&memory_lab, next_memory_topic()); /* first fit */
        ok = ok && push_back(&memory_lab, viewer_request());    /* split the rest */
    ...
    free(memory_lab.items);                   /* the destructor of std::vector would run here */
    return ok ? 0 : 1;
}
